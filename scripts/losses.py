...
from YOLO_DataGenerator import get_anchors
'''
Adapted from : https://github.com/maskaravivek/deep-learning/tree/master/Object%20Detection
'''
LAMBDA_COORD = 1
CLASS_SCALE = 1
...
ANCHORS = get_anchors('/home/user01/data_ssd/Talha/yolo_data/pascal_voc/train/').reshape(-1,1)
# ANCHORS = np.array([1.07709888,  1.78171903,  # anchor box 1, width , height
#                     2.71054693,  5.12469308,  # anchor box 2, width,  height
#                     10.47181473, 10.09646365,  # anchor box 3, width,  height
#                     5.48531347,  8.11011331,  # anchor box 4, width,  height
#                     9.47112   , 4.84053    ]) # anchor box 5, width,  height
...
CLASS_WEIGHTS    = np.ones(20, dtype='float32')
WARM_UP_BATCHES  = 100
def yolo_loss2(true_boxes):
    def custom_loss(y_true, y_pred):
        ...
        conf_mask = conf_mask + tf.cast(best_ious < 0.6, dtype= tf.float32) * (1 - y_true[..., 4]) * LAMBDA_NO_OBJECT
        ...
        Warm-up training
        """
        no_boxes_mask = tf.cast(coord_mask < LAMBDA_COORD/2., dtype= tf.float32)
        ...
        Finalize the loss
        """
        nb_coord_box = tf.math.reduce_sum(tf.cast(coord_mask > 0.0, dtype= tf.float32))
        nb_conf_box  = tf.math.reduce_sum(tf.cast(conf_mask  > 0.0, dtype= tf.float32))
        nb_class_box = tf.math.reduce_sum(tf.cast(class_mask > 0.0, dtype= tf.float32))
        ...
        nb_pred_box = tf.math.reduce_sum(tf.cast(true_box_conf > 0.5, dtype= tf.float32) * tf.cast(pred_box_conf > 0.3, dtype= tf.float32))
